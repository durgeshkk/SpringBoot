    ...
    public ResponseEntity<String> uploadFile(@RequestParam("Result") MultipartFile f){
        ...
        try{
            ...
            if(flag){
                ...
                // Trying to return the URL of the file uploaded
                // ServletUriComponentsBuilder.fromCurrentContextPath() will return the path of localhost8080
                // path will take to localhost8080/image/
                // then append file name & finally it will become the URL and will be send 
                // further
                // Now files will directly be saved in target itself
                // Only When we clean and build all the data is lost from target
                return ResponseEntity.ok(ServletUriComponentsBuilder.fromCurrentContextPath().path("/image/").path(f.getOriginalFilename()).toUriString());
            }
        }catch(Exception e){
            e.printStackTrace();
        }
